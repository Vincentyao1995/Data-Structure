...
def inputsampledata(testnumber):
    '''
...
def calrockpeaknum(libnumber, rocknumber):
    '''
    calculate the peak of the library infromation
    :param testnumber: from the library
    :return: peakcounts,  the peak number of the windows
             ...
             extermeminpeaks, the exterme minimum of the windows
    '''
    wavelengths, spectrum, name_list = inputsampledata(rocknumber)
    sample = np.array([wavelengths, spectrum]).T
    hull = qhull(sample)
    hull = changehull(hull, name_list)
    hull = pd.DataFrame(hull, columns=['wavelength', 'intensity'])
    ...
    lvalue = LRTable[libnumber][0]
    rvalue = LRTable[libnumber][1]
    print ('lvalue:', lvalue)
    print ('rvalue:', rvalue)
    ...
    def showval():
        ...
        plt.figure()
        plt.plot(wavelengths, newspectrum, c='k', label='Removed Spectrum')
        plt.scatter(wavelengths[plotpeak], newspectrum[plotpeak], c='r', label='Found peaks')
        plt.scatter(wavelengths[lvalue], newspectrum[lvalue], c='g', label='Left endpoints')
        plt.scatter(wavelengths[rvalue], newspectrum[rvalue], c='b', label='right endpoints')
        plt.legend()
...
def isPeakExist(libnumber, rockspectrum, libspectrum):
    ...
    for index in np.arange(len(lbands)):
        ...
        for peakindex in np.arange(len(rockbandpeakind)):
            depth = (rockspectrum[lbands[index]] + rockspectrum[rbands[index]])/2 - rockspectrum[rockbandpeakind[peakindex]]
    ...
    def calscalling_costantmindepth():
        ...
        def residuals(p, libextermeminpeaks):
            ...
            for item in np.arange(len(libextermeminpeaks)):
                rockdepth = (smoothrockspectrum[lbands[item]] + smoothrockspectrum[rbands[item]])/2 - smoothrockspectrum[libextermeminpeaks[item]]
        ...
        def residuals(p, libpeaklist):
            ...
            for item in np.arange(len(libpeaklist)):
                for t in libpeaklist[item]:
                    rockdepth = (smoothrockspectrum[lbands[item]] + smoothrockspectrum[rbands[item]]) / 2 - smoothrockspectrum[t]
        ...
        def residuals(p, libextermeminpeaks, predrockmin, libpeaklist):
            ...
            for index in np.arange(len(libextermeminpeaks)):
                ...
                for t in libpeaklist[index]:
                    rockdepth = (smoothrockspectrum[lbands[index]] + smoothrockspectrum[rbands[index]]) / 2 - smoothrockspectrum[t - distance]
...
def addnoise_spectrum(wavelengths, spectrum):
    scaling = eval(input('Please input the testing scaling number between 0-1.5 :'))
    shift = np.random.randn(len(spectrum)) / 100
    newspectrum = scaling * spectrum + shift

    # fig, axs = plt.subplots(1, 1, figsize=(10, 5))  # multiple plots in one figure
    plt.plot(wavelengths, spectrum, c='b', label = 'Original spectrum')
    plt.plot(wavelengths, newspectrum, label = 'noisy spectrum')
    plt.legend()
    # plt.show()
    return newspectrum
...
def singletest():
    ...
    libnumber = int(input("Please input the library test number:"))

    wavelengths, rockspectrum, rockname_list = inputsampledata(rocknumber)
    ...
    rockspectrum2 = getspectrum2(wavelengths, rockspectrum, rockname_list)

    # Calculate the lib peak information.
    print('Calculate the rock peak information:')
    calrockpeaknum(libnumber, rocknumber)
    isPeakExist(libnumber, rockspectrum2, libspectrum2)
    plt.show()

def tottest(rocknumber):
    # rocknumber = int(input('Please input the rock test number:'))
    libnumber = 0
    ...
    rocksmoothspectrum = signal.savgol_filter(rockspectrum, 31, 4, deriv=0)
    # plt.figure()
    ...
    libspectrum2 = getspectrum2(wavelengths, libspectrum, libname_list)
    rockspectrum2 = getspectrum2(wavelengths, rocksmoothspectrum, rockname_list)
...
if __name__ == "__main__":
    # test with only the library & all rock data
    # singletest()

    # if the scaling value is less than 0 then it also can say the peak doesn't exist.
    wavelengths, rockspectrum, rockname_list = inputsampledata(0)

    with open('test3.txt', 'wt') as f:
        for number in np.arange(25):
            flag, averagedistance, scaling, rms = tottest(number)
            if (flag == True):
                print ('Number %d has peaks.' %number)
                print ('Number %d has peaks.' %number, file=f)
                print ('Averagedistance %.3f' %averagedistance, file=f)
                print ('Scaling: %.3f; RMS: %.3f' %(scaling, rms), file=f)


            else:
                print('Number %d doesnot has peaks.' % number)
            def showanswer(number, averagedistance):
                plt.figure()
                wavelengths, rockspectrum, rockname_list = inputsampledata(number)
                rocksmoothspectrum = signal.savgol_filter(rockspectrum, 31, 4, deriv=0)
                rockspectrum2 = getspectrum2(wavelengths, rocksmoothspectrum, rockname_list)

                wavelengths, libspectrum, libname_list = inputData4Lib(0)
                libsmoothspectrum = signal.savgol_filter(libspectrum, 17, 4, deriv=0)
                libspectrum2 = getspectrum2(wavelengths, libsmoothspectrum, libname_list)

                plt.plot(wavelengths, rockspectrum, label='original spectrum')
                plt.plot(wavelengths, rocksmoothspectrum, label='smoothed spectrum')
                plt.scatter(wavelengths[LRTable[0][0]], rocksmoothspectrum[LRTable[0][0]], c='r', s=150,
                            label='left points')
                plt.scatter(wavelengths[LRTable[0][1]], rocksmoothspectrum[LRTable[0][1]], c='g', s=150,
                            label='right points')
                # plt.plot(wavelengths, rockspectrum2, c='k', label='removed spectrum')
                plt.legend()
                plt.figure()
                plt.title('The smoothing window number: %d, averagedistance: %.3f' % (number, averagedistance))
                plt.plot(wavelengths, rockspectrum2, label='removed spectrum')
                plt.plot(wavelengths, 0.9 + libspectrum2 * 0.1, label='library spectrum')
                plt.scatter(wavelengths[LRTable[0][0]], rockspectrum2[LRTable[0][0]], c='r', s=150, label='left points')
                plt.scatter(wavelengths[LRTable[0][1]], rockspectrum2[LRTable[0][1]], c='g', s=150, label='right points')
                plt.legend()
                plt.show()
            showanswer(number, averagedistance)
