...
def loaddata(filename):
    '''
    Load the important spectrums into the files.
    :return:
    '''
    lenslist, grouplist = loadidx(filename)
    lenslist2, grouplist2 = loadidx("../SWIR_Output/best-group-info1.txt")

    totexamplespectrums = np.loadtxt("../SWIR_Output/best-group-spectrums2.txt")
    ...
    for index in np.arange(len(grouplist)):
        ...
        for p in idx:
            if index != 0:
                spectrum = totexamplespectrums[p + lenslist2[index - 1]]
            else:
                spectrum = totexamplespectrums[p]
            ss.append(spectrum)
        # print ('data:', data)
        spectrums.extend(ss)

    print('loaddata all best spectrums done.')
    np.savetxt("../SWIR_Output/best-group-spectrums3.txt", spectrums)
...
def main():
    spectrums = np.loadtxt("../SWIR_Output/best-group-spectrums3.txt")
    print ('loadfiles done.')
    cluster_exemplars = [spectrums[0]]
    cluster_members_lists = [[]]
    # use the 0.04 angle threshold.
    ...
    pickle.dump(toSave, open('../SWIR/savefile004.pkl', 'wb'))
    f = open('../SWIR_Output/savefile4.txt', 'w')
...
def calculateRSquare():
    ...
    lenslist, grouplist = loadidx("../SWIR_Output/best-group-info2.txt")
    ...
    newdata = cs.savenewdata(sumlenslist, "../SWIR/savefile004.pkl", "../SWIR_Output/group_percents_elements3.csv", totsumlinelist)
    ...
    filename = "../SWIR_Output/group_percents_elements3.csv"
    dfgroupinfo = cs.calgroupnum(filename, sumlenslist, "../SWIR/savefile004.pkl", "../SWIR_Output/best-group-info3.txt")
    print('dfgroupinfo:', dfgroupinfo)


if __name__ == '__main__':
    # loadidx()
    # Save "best-group-spectrums3.txt"
    loaddata("../SWIR_Output/best-group-info2.txt")
    # Cluster and save "savefile004.pkl"
    main()
    # Save the new information of "best-group-info3.txt"
    calculateRSquare()
